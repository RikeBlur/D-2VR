...
def visualize_flow(flow, save_path=None):
    """Visualize optical flow as an RGB image using HSV color encoding.
    Args:
        flow: flow tensor, shape (1, H, W, 2) or (H, W, 2)
        save_path: if not None, save the image to this path
    Returns:
    ...
    hsv[..., 1] = 255
    hsv[..., 2] = np.clip(mag * 255.0 / (np.max(mag) + 1e-10), 0, 255).astype(np.uint8)
