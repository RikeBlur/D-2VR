...
@torch.no_grad()
def validate_chairs(model, iters=6, degradation_params=None):
    """ Perform evaluation on the FlyingChairs (test) split """
    model.eval()
    epe_list = []

    val_dataset = datasets.FlyingChairs(split='validation', degradation_params=degradation_params)
    
    if degradation_params is not None:
        print(f"Validation on FlyingChairs (DEGRADED mode)...")
    else:
        print(f"Validation on FlyingChairs (CLEAN mode)...")
    ...
    prefix = "Degraded " if degradation_params is not None else ""
    print(f"Validation Chairs {prefix}EPE: {epe:f}")
    
    key_name = 'chairs_epe_degraded' if degradation_params is not None else 'chairs_epe'
    return {key_name: epe}
...
@torch.no_grad()
def validate_things(model, iters=6, degradation_params=None):
    """ Perform evaluation on the FlyingThings (test) split """
    model.eval()
    results = {}

    if degradation_params is not None:
        print(f"Validation on FlyingThings3D (DEGRADED mode)...")

    for dstype in ['frames_cleanpass', 'frames_finalpass']:
        epe_list = []
        val_dataset = datasets.FlyingThings3D(dstype=dstype, split='validation', degradation_params=degradation_params)
        
        print(f'Dataset ({dstype}) length {len(val_dataset)}')
        for val_id in range(len(val_dataset)):
            image1, image2, flow_gt, _ = val_dataset[val_id]
            image1 = image1[None].cuda()
            image2 = image2[None].cuda()
        ...
        prefix = "Degraded " if degradation_params is not None else ""
        print(f"Validation ({dstype}) {prefix}EPE: {epe:f}, 1px: {px1:f}, 3px: {px3:f}, 5px: {px5:f}")
        
        key_suffix = '_degraded' if degradation_params is not None else ''
        results[dstype + key_suffix] = np.mean(epe_list)
...
@torch.no_grad()
def validate_sintel(model, iters=6, degradation_params=None):
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}
    
    if degradation_params is not None:
        print(f"Validation on Sintel (DEGRADED mode)...")

    for dstype in ['clean', 'final']:
        val_dataset = datasets.MpiSintel(split='training', dstype=dstype, degradation_params=degradation_params)
        epe_list = []
...
@torch.no_grad()
def validate_sintel_occ(model, iters=6, degradation_params=None):
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}
    
    if degradation_params is not None:
        print(f"Validation on Sintel OCC (DEGRADED mode)...")

    for dstype in ['albedo', 'clean', 'final']:
    # for dstype in ['clean', 'final']:
        val_dataset = datasets.MpiSintel(split='training', dstype=dstype, occlusion=True, degradation_params=degradation_params)
        epe_list = []
        epe_occ_list = []
        epe_noc_list = []
        ...
        for val_id in range(len(val_dataset)):
            ...
            image1 = data_blob[0][None].cuda()
            image2 = data_blob[1][None].cuda()
            flow_gt = data_blob[2]
            # valid = data_blob[3]
            occ = data_blob[4]
        ...
        prefix = "Degraded " if degradation_params is not None else ""
        print(f"Validation ({dstype}) {prefix}EPE: {epe:f}, 1px: {px1:f}, 3px: {px3:f}, 5px: {px5:f}")
        print(f"Occ epe: {epe_occ_mean:f}, Noc epe: {epe_noc_mean:f}")
...
@torch.no_grad()
def validate_kitti(model, iters=6, degradation_params=None):
    """ Peform validation using the KITTI-2015 (train) split """
    model.eval()
    
    val_dataset = datasets.KITTI(split='training', degradation_params=degradation_params)

    if degradation_params is not None:
        print(f"Validation on KITTI (DEGRADED mode)...")
    ...
    prefix = "Degraded " if degradation_params is not None else ""
    print(f"Validation KITTI {prefix}: {epe:f}, {f1:f}")
    
    key_suffix = '_degraded' if degradation_params is not None else ''
    return {f'kitti_epe{key_suffix}': epe, f'kitti_f1{key_suffix}': f1}
