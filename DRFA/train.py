...
def main(args):
    ...
    degradation_config = {
        ...
        'second_order': True,
        "blur_kernel_size_2": [1, 3],
        "noise_sigma_2": [1.0, 5.0],
        "jpeg_quality_2": [70, 90]
    }

    if args.stage == 'kitti':
        args.degradation_params = None 
    else:
        args.degradation_params = degradation_config 
...
def validate(model, args, logger):
    ...
    val_deg_params = getattr(args, 'degradation_params', None)

    # Evaluate results
    for val_dataset in args.validation:
        if val_dataset == 'chairs':
            results.update(evaluate.validate_chairs(model.module, args.iters, degradation_params=val_deg_params))
        elif val_dataset == 'sintel':
            results.update(evaluate.validate_sintel(model.module, args.iters, degradation_params=val_deg_params))
        elif val_dataset == 'kitti':
            results.update(evaluate.validate_kitti(model.module, args.iters, degradation_params=val_deg_params))
