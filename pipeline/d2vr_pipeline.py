...
from diffusers.schedulers import EulerAncestralDiscreteScheduler, DDPMScheduler, LMSDiscreteScheduler
...
import opticalflow.flow_utils as of
...
class D2VRPipeline(
    ...
    def _print_model_info(self):
        ...
        for name, model in models_info.items():
            if model is not None:
                total, trainable, memory = count_params(model)
                total_all += total
    ...
    def enable_model_cpu_offload(self, gpu_id=1):
        r"""
    ...
    def __call__(
        ...
        of_vis_path: Optional[str] = None,
        seq_name: Optional[str] = None,
        of_random: bool = False
    ):
        r"""
        ...
        base_latent = self.prepare_latents(
        ...
        for _ in range(len(images)):
            #perturbed_latent = base_latent + noise_scale * torch.randn_like(base_latent)
            #perturbed_latent = perturbed_latent * self.scheduler.init_noise_sigma / perturbed_latent.std()
        ...
        with self.progress_bar(total=len(timesteps)*len(images)) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if of_random:
                    flows = [torch.randn_like(flow) for flow in forward_flows]
                else:
                    flows = forward_flows if not reversed else backward_flows
                
                if i == 0 and of_vis_path is not None and seq_name is not None:
                    import os
                    from opticalflow.flow_utils import visualize_flow
                    vis_dir = os.path.join(of_vis_path, seq_name)
                    os.makedirs(vis_dir, exist_ok=True)
                    
                    num_flows_to_save = min(10, len(flows))
                    for idx in range(num_flows_to_save):
                        flow_path = os.path.join(vis_dir, f'flow_{idx:03d}.png')
                        visualize_flow(flows[idx], flow_path)
